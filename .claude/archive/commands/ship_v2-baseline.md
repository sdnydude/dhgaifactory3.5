...
## Phase 0: Feedback Briefing

**Goal:** Load behavioral context from the memreg capture pipeline before planning begins. This prevents repeating known anti-patterns.

1. Query the registry for recent feedback data (all calls fire-and-forget — if the API is down, skip silently):

```bash
CORRECTIONS=$(curl -sf --max-time 3 http://10.0.0.251:8011/api/corrections/stats 2>/dev/null)
DEFERRED=$(curl -sf --max-time 3 http://10.0.0.251:8011/api/deferred-items/stats 2>/dev/null)
HEALTH=$(curl -sf --max-time 3 http://10.0.0.251:8011/api/feedback-loop/health 2>/dev/null)
```

2. Summarize in 3-5 lines:
   - **Active correction patterns** (categories with repeat_flag=true in last 7 days) — these are behavioral traps to avoid
   - **Open deferred items** related to the feature area (match by category/tags to the feature description)
   - **Pipeline health** status (healthy/degraded)

3. If active repeat patterns exist, state: "Correction patterns active: [list]. I'll be especially careful about [specific behavior] during this ship."

4. If related deferred items exist, note them: "There are [N] open deferred items in [category] that may be relevant to this work."

5. Proceed to Phase 1 immediately — no approval gate here.
