...
- [x] **`/outputs` endpoint document_text passthrough** (Apr 13, #52) — `AgentOutput` Pydantic schema in `cme_endpoints.py` and frontend `AgentOutput` interface in `types/cme.ts` were both missing the field even though `models.py:445` defines it and `_sync_project_from_thread` writes it. Threaded `document_text=o.document_text` through both list and single-fetch endpoints. 3 new real-DB integration tests in `TestCMEOutputsRealDB`. Verified live against ef4e5b53: 6 outputs all populated (research=26,809 chars, clinical=13,803, gap_analysis=16,415, learning_objectives=7,455, needs_assessment=16,532, prose_quality_1=91). Memory saved as `feedback_serializer_drift.md` — when DB has data but API doesn't, suspect serializer drift before migration.
