...
For the post-R0 responsive shell (AppShell, Sidebar, TopBar, tab bar), see [Responsive Shell](/portage/frontend/responsive-shell).
...
The root `app/layout.tsx` wraps everything in [`AppShell`](/portage/frontend/responsive-shell) — the route-aware responsive shell (desktop sidebar + top bar at `lg+`, floating glass tab bar on mobile). `TabBar` mounts once inside `AppShell` for all non-admin routes; `(tabs)/layout.tsx` only adds bottom padding and `PorterProvider`.
