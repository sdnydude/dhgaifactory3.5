...
Returns Prometheus-formatted metrics for scraping (guarded by a `METRICS_SECRET` bearer token when configured, not by the admin role). See [Monitoring](/portage/monitoring) for the full metrics list.
