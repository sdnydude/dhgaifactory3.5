...
Values for all of these live in Doppler — see [Secrets & Storage](/portage/infrastructure/secrets-and-storage).

For the full session-exchange contract from the client's perspective, see [Authentication API](/portage/api/authentication). For the rest of the host layout, see [Infrastructure Overview](/portage/infrastructure/overview).
