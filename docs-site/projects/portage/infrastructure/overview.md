...
The five services below are defined in `docker-compose.yml` at the repo root. Per-service operational detail (rebuild rituals, log commands, gotchas) lives in the [Service Runbook](/portage/infrastructure/services).
...
The Express 5 backend, built from `apps/api/Dockerfile` and published on `:8016`. It serves **HTTPS with a self-signed certificate** (SAN `10.0.0.251`) — the `certs/` directory is mounted read-only into the container, and the container's own healthcheck probes `https://localhost:8016/health` with `rejectUnauthorized: false`. Runtime configuration comes from `env_file: .env` (synced from Doppler — see [Secrets & Storage](/portage/infrastructure/secrets-and-storage)). The service carries Prometheus scrape labels (`prometheus.io/scrape: "true"`, port `8016`, path `/metrics`) and is the only service attached to **both** `portage-network` and the external `dhg-network` (see [Networks](#networks) below).
...
|-------|------|------------------|------------------|
| portage-db | 127.0.0.1:5436 → 5432 | `docker compose up -d` (stock image + `portage-pgdata` volume) | [Service Runbook](/portage/infrastructure/services#portage-db) |
| portage-api | 8016 (HTTPS) | `docker compose up -d --build portage-api` (image-baked) | [Service Runbook](/portage/infrastructure/services#portage-api) |
| portage-app | 3002 → 3000 | `docker compose up -d --build portage-app` (image-baked) | [Service Runbook](/portage/infrastructure/services#portage-app) |
| portage-rembg | 7000 | `docker compose up -d` (upstream image) | [Service Runbook](/portage/infrastructure/services#portage-rembg) |
| portage-graph | 8018 → 80 | `docker compose up -d`; content updates via graphify regeneration, no restart | [Service Runbook](/portage/infrastructure/services#portage-graph) |
| dhg-docs | 8017 → 80 | GitHub Actions workflow restart/recreate | [CI/CD & Docs Pipeline](/portage/infrastructure/ci-cd) |
| DHG Registry | 8011 | Separate AI Factory stack | [Registry Integration](/portage/development/registry-integration) |
| Cloudflare Tunnel + Access | — (outbound tunnel) | `infra/cloudflared/` config | [Cloudflare Tunnel & Access](/portage/infrastructure/cloudflare) |
| Doppler / R2 | — | SessionStart sync / R2 bucket | [Secrets & Storage](/portage/infrastructure/secrets-and-storage) |
| GitHub Actions runner | — | systemd service on g700data1 | [CI/CD & Docs Pipeline](/portage/infrastructure/ci-cd) |

For the standard deploy rituals and build commands shared across services, see [Deployment](/portage/deployment).
