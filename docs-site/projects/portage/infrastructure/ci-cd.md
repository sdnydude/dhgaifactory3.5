...
- `](/portage/` → `](/portage/` — in the repo, docs cross-reference each other with absolute `/docs/...` links; on the deployed shared site, Portage docs live under the `/portage/` route prefix instead. Without this rewrite every internal link would 404.
- `](/portage/img/` → `](/portage/img/` — image assets are copied to `static/portage/img` on the shared site (served at `/portage/img/...`), but repo docs reference `/img/...`. Without this rewrite every image would 404 on the deployed site.
...
The application containers are **not** CI-deployed. `portage-api` and `portage-app` run as image-baked containers (no source bind mounts), and deployment is an operator action on the host: `docker compose up -d --build <service>`. Pushing to `main` does not redeploy the app — only the docs pipeline is automated. Hot-reload development is an explicit opt-in overlay (`docker-compose.dev.yml`); the default is always the baked prod image. See [Deployment](/portage/deployment) for the full procedure and [Infrastructure Overview](/portage/infrastructure/overview) for the host layout.
