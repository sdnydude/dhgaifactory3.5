...
| eBay publishing | Trading API ("Trade-First") over Inventory API | Inline shipping terms remove the Business Policies setup gate; one `AddFixedPriceItem` call replaces the inventory_item → offer → publish three-step with no silent offer-state failures — see [Marketplace Adapters](/portage/architecture/marketplace-adapters) and [eBay Trade-First Publishing](/portage/reference/ebay-trade-first) |

## Deep dives

- [Database Schema](/portage/architecture/database)
- [Marketplace Adapters](/portage/architecture/marketplace-adapters)
- [AI Pipeline](/portage/architecture/ai-pipeline)
- [App Sitemap](/portage/architecture/sitemap)
