...
55 rules: **17 with automated diagnostics** (`observability/runbooks/<alert>.yml`, read-only, run by dhg-remediator when the incident opens) and **38 human-only** (the section on this page is the whole runbook). Only `critical` and `high` alerts open incidents, so the `warning` rows marked automated never actually reach the remediator; they are documented for the harness and for the day the severity changes. Generated by `observability/scripts/gen-runbooks.py`; edit the YAML, not this table.

| Alert | Severity | Coverage | Trigger | Diagnostics |
|---|---|---|---|---|
| [ContainerCrashLoop](#containercrashloop) | `high` | human-only | — | — |
| [HostMemoryHigh](#hostmemoryhigh) | `critical` | automated diagnostics | `T3` | 3 |
| [HostSwapHigh](#hostswaphigh) | `high` | automated diagnostics | `T4` | 3 |
| [RootDiskHigh](#rootdiskhigh) | `high` | automated diagnostics | `T5` | 3 |
| [DataDiskHigh](#datadiskhigh) | `warning` | automated diagnostics (warning: no incident) | `T6` | 2 |
| [PrometheusTargetDown](#prometheustargetdown) | `high` | automated diagnostics | `T8` | 3 |
| [OllamaDown](#ollamadown) | `high` | automated diagnostics | `T18` | 4 |
| [PortageApiDown](#portageapidown) | `high` | automated diagnostics | `T19` | 3 |
| [RegistryApiDown](#registryapidown) | `critical` | human-only | — | — |
| [ContainerMemoryLeak](#containermemoryleak) | `high` | automated diagnostics | `T13` | 3 |
| [ContainerHighCPU](#containerhighcpu) | `warning` | human-only | — | — |
| [ContainerHighMemory](#containerhighmemory) | `warning` | automated diagnostics (warning: no incident) | `T24` | 3 |
| [AlertmanagerDown](#alertmanagerdown) | `critical` | automated diagnostics | `T20` | 3 |
| [PrometheusRuleEvalFailures](#prometheusruleevalfailures) | `high` | human-only | — | — |
| [PrometheusNotificationsDropped](#prometheusnotificationsdropped) | `high` | human-only | — | — |
| [AlertmanagerNotificationsFailing](#alertmanagernotificationsfailing) | `high` | human-only | — | — |
| [LokiRulerErrors](#lokirulererrors) | `warning` | human-only | — | — |
| [TextfileStale](#textfilestale) | `warning` | human-only | — | — |
| [PortageHighErrorRate](#portagehigherrorrate) | `high` | human-only | — | — |
| [PortageHighLatency](#portagehighlatency) | `warning` | human-only | — | — |
| [MemregDLQBacklog](#memregdlqbacklog) | `high` | human-only | — | — |
| [MemregDLQMetricMissing](#memregdlqmetricmissing) | `warning` | human-only | — | — |
| [LokiStoreGrowth](#lokistoregrowth) | `high` | automated diagnostics | `T15` | 3 |
| [LokiDown](#lokidown) | `critical` | automated diagnostics | `T17` | 3 |
| [AlloyDown](#alloydown) | `high` | automated diagnostics | `T16` | 4 |
| [BackupStale](#backupstale) | `critical` | human-only | — | — |
| [BackupFailed](#backupfailed) | `high` | human-only | — | — |
| [RestoreDrillStale](#restoredrillstale) | `high` | human-only | — | — |
| [BackupOffsiteStale](#backupoffsitestale) | `high` | human-only | — | — |
| [Dh40801Down](#dh40801down) | `critical` | human-only | — | — |
| [Dh40801DiskHigh](#dh40801diskhigh) | `high` | human-only | — | — |
| [LangfuseUnhealthy](#langfuseunhealthy) | `critical` | human-only | — | — |
| [LangfuseCanaryStale](#langfusecanarystale) | `critical` | human-only | — | — |
| [LangfuseContainerRestart](#langfusecontainerrestart) | `high` | human-only | — | — |
| [GpuVramHigh](#gpuvramhigh) | `warning` | human-only | — | — |
| [GpuTempHigh](#gputemphigh) | `high` | human-only | — | — |
| [NasDown](#nasdown) | `critical` | human-only | — | — |
| [NasRaidCrashed](#nasraidcrashed) | `critical` | human-only | — | — |
| [NasRaidDegraded](#nasraiddegraded) | `high` | human-only | — | — |
| [NasDiskUnhealthy](#nasdiskunhealthy) | `critical` | human-only | — | — |
| [NasVolumeHigh](#nasvolumehigh) | `warning` | human-only | — | — |
| [NasTempHigh](#nastemphigh) | `warning` | human-only | — | — |
| [PostgresDown](#postgresdown) | `critical` | human-only | — | — |
| [PostgresConnectionsHigh](#postgresconnectionshigh) | `high` | automated diagnostics | `T9` | 3 |
| [FrontendDown](#frontenddown) | `high` | automated diagnostics | `T21` | 3 |
| [OpenWebUIDown](#openwebuidown) | `high` | automated diagnostics | `T22` | 4 |
| [GrafanaDown](#grafanadown) | `high` | automated diagnostics | `T23` | 3 |
| [RegistryPublicDown](#registrypublicdown) | `warning` | human-only | — | — |
| [RegistryHighErrorRate](#registryhigherrorrate) | `high` | human-only | — | — |
| [RegistryHighLatency](#registryhighlatency) | `warning` | human-only | — | — |
| [HighErrorRate](#higherrorrate) | `warning` | human-only | — | — |
| [ContainerErrorSpike](#containererrorspike) | `warning` | human-only | — | — |
| [PostgresFatalError](#postgresfatalerror) | `critical` | human-only | — | — |
| [NoLogsFromRegistryApi](#nologsfromregistryapi) | `warning` | human-only | — | — |
| [SecretLeakDetected](#secretleakdetected) | `critical` | human-only | — | — |
{/* coverage:end */}
...
{/* automation:begin HostMemoryHigh */}
**Automation** — `observability/runbooks/HostMemoryHigh.yml`, trigger `T3`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Top container memory consumers right now | `docker stats --no-stream --format 'table {{.Name}}\t{{.MemUsage}}\t{{.MemPerc}}'` |
| 2 | Host available vs total memory | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=node_memory_MemAvailable_bytes / node_memory_MemTotal_bytes'` |
| 3 | Top 10 containers by memory as cAdvisor sees them | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=topk(10, container_memory_usage_bytes{name=~"dhg-.*"})'` |
{/* automation:end */}
...
{/* automation:begin HostSwapHigh */}
**Automation** — `observability/runbooks/HostSwapHigh.yml`, trigger `T4`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Swap free vs total | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=node_memory_SwapFree_bytes / node_memory_SwapTotal_bytes'` |
| 2 | Host available memory | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=node_memory_MemAvailable_bytes'` |
| 3 | Top container memory consumers right now | `docker stats --no-stream --format 'table {{.Name}}\t{{.MemUsage}}\t{{.MemPerc}}'` |
{/* automation:end */}
...
{/* automation:begin RootDiskHigh */}
**Automation** — `observability/runbooks/RootDiskHigh.yml`, trigger `T5`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Root filesystem free vs size | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=node_filesystem_avail_bytes{mountpoint="/"} / node_filesystem_size_bytes{mountpoint="/"}'` |
| 2 | Growth over the last 24h | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=node_filesystem_avail_bytes{mountpoint="/"} offset 24h - node_filesystem_avail_bytes{mountpoint="/"}'` |
| 3 | Exited containers still holding writable layers | `docker ps -a --filter status=exited --format '{{.Names}} {{.Status}} {{.Size}}'` |
{/* automation:end */}
...
{/* automation:begin DataDiskHigh */}
**Automation** — `observability/runbooks/DataDiskHigh.yml`, trigger `T6`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | /mnt/4tb free vs size | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=node_filesystem_avail_bytes{mountpoint="/mnt/4tb"} / node_filesystem_size_bytes{mountpoint="/mnt/4tb"}'` |
| 2 | Loki store size | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=loki_store_bytes'` |

Severity `warning`: the registry webhook opens no incident, so these diagnostics run only if the severity is raised.

Notes: Severity warning - no incident is opened; these diagnostics run only if the severity is raised.
{/* automation:end */}
...
{/* automation:begin PrometheusTargetDown */}
**Automation** — `observability/runbooks/PrometheusTargetDown.yml`, trigger `T8`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Current up value and labels for the failing job | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=up{job="{job}"}'` |
| 2 | Every unhealthy target with its last scrape error | `curl -s http://dhg-prometheus:9090/api/v1/targets?state=unhealthy` |
| 3 | Containers that have exited | `docker ps -a --filter status=exited --format '{{.Names}} {{.Status}}'` |
...
{/* automation:begin OllamaDown */}
**Automation** — `observability/runbooks/OllamaDown.yml`, trigger `T18`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Container state, exit code and restart count | `docker inspect dhg-ollama --format '{{.State.Status}} exit={{.State.ExitCode}} restarts={{.RestartCount}} started={{.State.StartedAt}}'` |
| 2 | Last 100 log lines | `docker logs --tail 100 dhg-ollama` |
| 3 | Container-path API answer (model list) | `curl -s -m 10 http://dhg-ollama:11434/api/tags` |
| 4 | Both probe paths as Prometheus sees them | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=probe_success{service="ollama"}'` |

Notes: Every minute down is paid cloud inference for Portage Porter.
{/* automation:end */}
...
{/* automation:begin PortageApiDown */}
**Automation** — `observability/runbooks/PortageApiDown.yml`, trigger `T19`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Container state, exit code and restart count | `docker inspect portage-api --format '{{.State.Status}} exit={{.State.ExitCode}} restarts={{.RestartCount}} started={{.State.StartedAt}}'` |
| 2 | Last 200 log lines | `docker logs --tail 200 portage-api` |
| 3 | Direct scrape vs public probe (which leg is failing) | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=up{job="portage-api"} or probe_success{service="portage-api"}'` |
...
{/* automation:begin ContainerMemoryLeak */}
**Automation** — `observability/runbooks/ContainerMemoryLeak.yml`, trigger `T13`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Live memory usage of the container | `docker stats --no-stream --format '{{.Name}} {{.MemUsage}} {{.MemPerc}}' {container}` |
| 2 | Configured memory limit (0 means none) | `docker inspect {container} --format 'limit={{.HostConfig.Memory}} restarts={{.RestartCount}} started={{.State.StartedAt}}'` |
| 3 | Usage over the last 24h (flat = loaded once, rising = leak) | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=container_memory_usage_bytes{name="{container}"} - container_memory_usage_bytes{name="{container}"} offset 24h'` |
{/* automation:end */}
...
{/* automation:begin ContainerHighMemory */}
**Automation** — `observability/runbooks/ContainerHighMemory.yml`, trigger `T24`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Live memory usage against the limit | `docker stats --no-stream --format '{{.Name}} {{.MemUsage}} {{.MemPerc}}' {container}` |
| 2 | Configured limit and restart count (OOM kills show up as restarts) | `docker inspect {container} --format 'limit={{.HostConfig.Memory}} oomkilled={{.State.OOMKilled}} restarts={{.RestartCount}}'` |
| 3 | Usage-to-limit ratio as Prometheus sees it | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=container_memory_usage_bytes{name="{container}"} / container_spec_memory_limit_bytes{name="{container}"}'` |
...
{/* automation:begin AlertmanagerDown */}
**Automation** — `observability/runbooks/AlertmanagerDown.yml`, trigger `T20`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Container state, exit code and restart count | `docker inspect dhg-alertmanager --format '{{.State.Status}} exit={{.State.ExitCode}} restarts={{.RestartCount}} started={{.State.StartedAt}}'` |
| 2 | Last 100 log lines (an invalid rendered config is named on the first error line) | `docker logs --tail 100 dhg-alertmanager` |
| 3 | Health endpoint from inside the network | `curl -s -m 5 http://dhg-alertmanager:9093/-/healthy` |
...
{/* automation:begin PostgresConnectionsHigh */}
**Automation** — `observability/runbooks/PostgresConnectionsHigh.yml`, trigger `T9`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Backends vs max_connections for the instance | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=sum by (service) (pg_stat_database_numbackends{service="{service}"}) / max by (service) (pg_settings_max_connections{service="{service}"})'` |
| 2 | Backends per database on the instance | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=pg_stat_database_numbackends{service="{service}"}'` |
| 3 | Backend growth over the last hour | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=sum by (service) (pg_stat_database_numbackends{service="{service}"}) - sum by (service) (pg_stat_database_numbackends{service="{service}"} offset 1h)'` |
...
{/* automation:begin FrontendDown */}
**Automation** — `observability/runbooks/FrontendDown.yml`, trigger `T21`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Container state, exit code and restart count | `docker inspect dhg-frontend --format '{{.State.Status}} exit={{.State.ExitCode}} restarts={{.RestartCount}} started={{.State.StartedAt}}'` |
| 2 | Last 100 log lines | `docker logs --tail 100 dhg-frontend` |
| 3 | HTTP status from inside the network | `curl -s -m 10 -o /dev/null -w '%{http_code}' http://dhg-frontend:3000/` |
{/* automation:end */}
...
{/* automation:begin OpenWebUIDown */}
**Automation** — `observability/runbooks/OpenWebUIDown.yml`, trigger `T22`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Container state, exit code and restart count | `docker inspect dhg-open-webui --format '{{.State.Status}} exit={{.State.ExitCode}} restarts={{.RestartCount}} started={{.State.StartedAt}}'` |
| 2 | Last 100 log lines | `docker logs --tail 100 dhg-open-webui` |
| 3 | HTTP status from inside the network | `curl -s -m 10 -o /dev/null -w '%{http_code}' http://dhg-open-webui:8080/` |
| 4 | Ollama dependency answering | `curl -s -m 10 -o /dev/null -w '%{http_code}' http://dhg-ollama:11434/api/tags` |
{/* automation:end */}
...
{/* automation:begin GrafanaDown */}
**Automation** — `observability/runbooks/GrafanaDown.yml`, trigger `T23`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Container state, exit code and restart count | `docker inspect dhg-grafana --format '{{.State.Status}} exit={{.State.ExitCode}} restarts={{.RestartCount}} started={{.State.StartedAt}}'` |
| 2 | Last 100 log lines (an invalid provisioning file is named on the first error) | `docker logs --tail 100 dhg-grafana` |
| 3 | Health endpoint from inside the network | `curl -s -m 5 http://dhg-grafana:3000/api/health` |
{/* automation:end */}
...
Rules from `observability/prometheus/rules.d/backups.yml`. The series come from
`/mnt/4tb/observability/textfile/backups.prom`, written by
`observability/scripts/backup-all.sh` (nightly 03:30 ET, all 13 targets, then
the NAS mirror) and `observability/scripts/restore-drill.sh` (Sundays 04:00 ET),
scraped by the g700data1 node-exporter. Every per-target series carries
`name="<target>"`; every rule sets `service="backups"`, which gives one registry
incident per target and keeps Alertmanager grouping separate from node-exporter.
All four alerts are human-only: the remediator's allowlist cannot run `gpg`,
`rsync` or a restore, and nothing here should be automated anyway. Design and
restore procedures: [Backups](../backups.md).

### BackupStale

```promql
(time() - backup_last_success_timestamp > 100800) or absent(backup_last_success_timestamp)
```

**Means:** no verified, manifest-bearing backup run of `{{name}}` in 28 hours
(one missed night plus slack), or the `backups.prom` textfile is missing
entirely (the `absent()` clause: a deleted textfile must page, not go quiet).

**First three checks:**

```bash
tail -50 ~/.claude/run/backup-all.log
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=time() - backup_last_success_timestamp'
ls -la /mnt/4tb/backups/nightly/<target>/ | tail -5
```

**Likely causes:** the cron line is gone or cron did not run (`crontab -l`);
Doppler unreachable at 03:30 so the passphrase guard aborted the whole run;
`/mnt/4tb` under the 10 GB free-space guard; a target failing every night
(then `BackupFailed` fired first — read that one).

**Resolve:** run the target by hand and read the failure line:
`doppler run --project dhg-monitoring --config dev -- observability/scripts/backup-all.sh --target <name>`.
If every target is stale, the run itself is not starting: check cron, Doppler,
and free space, then run `backup-all.sh` with no arguments.

**Dashboard:** [Backups & NAS](http://10.0.0.251:3001/d/dhg-platform-backups)
...
```promql
backup_last_attempt_timestamp > backup_last_success_timestamp
```

**Means:** the most recent attempt for `{{name}}` did not complete. The script
stamps `attempt` before producing and `success` only after the manifest lands,
so this fires the same morning a target fails, while `BackupStale` waits 28 h.
The previous good run is still on disk and on the NAS.

**First three checks:**

```bash
grep -E 'target <name>|FAILED' ~/.claude/run/backup-all.log | tail -5
docker ps --format '{{.Names}}\t{{.Status}}' | grep -E '<source container>'
docker --context dh40801 version >/dev/null && echo dh40801 reachable
```

**Likely causes:** the source container is stopped or unhealthy; dh40801 is
unreachable (its three targets fail together after one 30 s probe); a config
file listed in `observability/scripts/config-layer.list` became unreadable
(the config target fails loudly on purpose); an integrity check failed
(`pg_restore --list`, `tar -t`, `gzip -t`) — which means the producer streamed
garbage, not that the disk is bad.

**Resolve:** fix the source, then re-run just that target:
`doppler run --project dhg-monitoring --config dev -- observability/scripts/backup-all.sh --target <name> --no-offsite`.
A successful re-run moves `success` past `attempt` and the alert resolves.
...
```promql
(time() - backup_restore_drill_success_timestamp > 691200) or absent(backup_restore_drill_success_timestamp)
```

**Means:** `restore-drill.sh` has not restored and verified `{{name}}` into an
ephemeral container in 8 days (one missed Sunday plus slack), or never. A backup
nobody has restored is a hope, not a backup.

**First three checks:**

```bash
tail -50 ~/.claude/run/restore-drill.log
docker ps -a --format '{{.Names}}' | grep '^dhg-restore-drill-'    # must be empty between drills
docker images --format '{{.Repository}}:{{.Tag}}' | grep -E 'postgres:17|clickhouse-server:25.12|chainguard/minio|dhg-drill-sqlite'
```

**Likely causes:** the Sunday cron line is missing; a drill image was pruned
from g700data1 (`docker system prune` removes the pre-pulled `postgres:17`,
`clickhouse-server:25.12`, `chainguard/minio`, `dhg-drill-sqlite:3.20`); a real
restore defect — counts no longer match the manifest, a Native stream no longer
inserts after a Langfuse upgrade, MinIO refuses the archived credentials.

**Resolve:** run the drill for the named target and read its verdict:
`doppler run --project dhg-monitoring --config dev -- observability/scripts/restore-drill.sh <name>`.
A count mismatch or a restore error is a real finding: the backup of that target
is not restorable as-is. Fix the producer or the drill, re-run the backup, drill
again. Never "fix" it by loosening the equality check.
...
```promql
(time() - backup_offsite_last_success_timestamp > 100800) or absent(backup_offsite_last_success_timestamp)
```

**Means:** the nightly `rsync` of `/mnt/4tb/backups/nightly/` to the Synology
(`aifactory-backup@10.0.0.250::aifactory-backups`) has not succeeded in 28 h.
Local backups are still being written; only the off-host copy is behind, so the
same-device risk (backups and Docker volumes share the `/mnt/4tb` NVMe) is live
until this clears.

**First three checks:**

```bash
grep -E 'offsite' ~/.claude/run/backup-all.log | tail -3
rsync --list-only -e 'ssh -p 22 -i ~/.ssh/nas-backup_ed25519 -o IdentitiesOnly=yes -o BatchMode=yes' aifactory-backup@10.0.0.250::aifactory-backups/ | head
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=up{job="nas"}'
```

**Likely causes:** the NAS is down (`NasDown` fires too); the DSM rsync service
was disabled (Control Panel > File Services > rsync — SSH-encrypted mode needs it
on, which is why port 873 stays listening); the `homes` share was set to No
Access for `aifactory-backup` (sshd then cannot read its `authorized_keys`);
the key `~/.ssh/nas-backup_ed25519` was removed; the NAS volume is full or
degraded.

**Resolve:** restore the DSM setting or share permission, prove the transport
with the `rsync --list-only` line above, then re-run
`backup-all.sh` (the mirror is idempotent; `--delete-delay` keeps a killed
mirror from deleting before it has updated).
...
Rules from `observability/prometheus/rules.d/nas.yml`. The Synology DS1618+
(`10.0.0.250`, DSM 7.1.1) is scraped over SNMPv3 by `dhg-snmp-exporter`
(`prom/snmp-exporter`, module `synology`, job `nas`). Series are the MIB object
names: `raidStatus{raidName}`, `diskStatus{diskID}`, `diskHealthStatus`,
`diskTemperature`, `raidFreeSize`, `raidTotalSize`, plus `up{job="nas"}`.
`raidStatus` values (SYNOLOGY-RAID-MIB): 1 Normal, 2-10 transitions (repairing,
syncing, parity check), 11 Degrade, 12 Crashed, 13 DataScrubbing, 14-21 other
transitions. Only 11 and 12 alert. All six alerts are human-only: drives are
replaced by a person. This NAS ran at 4 of 6 disks from 2026-02-25 to
2026-09-10 with nobody knowing; these rules exist so that never repeats.
...
**Means:** `dhg-snmp-exporter` could not get an SNMP answer from `10.0.0.250`
for 5 minutes. Either the NAS is off/unreachable or SNMPv3 was disabled.

**First three checks:**

```bash
ping -c 2 10.0.0.250
curl -s 'http://10.0.0.251:9090/api/v1/targets' | python3 -c 'import sys,json;[print(t["health"],t["lastError"]) for t in json.load(sys.stdin)["data"]["activeTargets"] if t["labels"].get("job")=="nas"]'
docker logs --tail 20 dhg-snmp-exporter
```

**Likely causes:** NAS powered off or rebooting (DSM update); SNMP service
disabled or SNMPv3 user changed in DSM (Control Panel > Terminal & SNMP);
`observability/snmp-exporter/auths.yml` not rendered after a Doppler change;
LAN/VLAN change.

**Resolve:** if the NAS is up, re-render the auths file
(`observability/scripts/render-snmp-exporter.sh`), restart `dhg-snmp-exporter`,
and confirm SNMPv3 is on in DSM. Until it clears, `BackupOffsiteStale` will
follow within 28 h.
...
Data on the volume is unavailable and at risk. Critical.

**First three checks:**

```bash
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=raidStatus'
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=diskStatus'
doppler run --project dhg-monitoring --config dev -- bash -c 'sshpass -e ssh -p 28 "$SYNOLOGY_OPS_USER@$SYNOLOGY_HOST" cat /proc/mdstat'   # SSHPASS from SYNOLOGY_OPS_PASSWORD
```

**Likely causes:** a second (RAID 5) or third (RAID 6) disk failure; a
backplane or SATA fault taking several bays offline at once; a controller
reset.

**Resolve:** do not reboot, do not pull drives. Storage Manager shows which
bays; Synology's crashed-pool procedure applies. The backups on
`/mnt/4tb/backups/nightly` are the copy that matters now; keep `backup-all.sh`
running with `--no-offsite` until the pool is back.
...
at 5 of 6 tolerates one more failure; at 4 of 6 it tolerates none. High, not
critical, on purpose: a degrade can last weeks while a drive ships, and a
critical here would inhibit `NasTempHigh`/`NasVolumeHigh` for the whole period
(inhibit rule 1 matches on `service`).

**First three checks:**

```bash
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=raidStatus'
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=diskStatus != 1 or diskHealthStatus > 1'
doppler run --project dhg-monitoring --config dev -- bash -c 'sshpass -e ssh -p 28 "$SYNOLOGY_OPS_USER@$SYNOLOGY_HOST" "cat /proc/mdstat; for d in sda sdb sdc sdd sde sdf; do [ -e /sys/block/\$d ] && echo \$d present || echo \$d ABSENT; done"'
```

**Likely causes:** a disk failed or was deactivated by DSM (the September 2026
case: Drive 2 kicked as "non-fresh" at boot); a bay lost its SATA link; a drive
was pulled.

**Resolve:** Storage Manager > HDD/SSD to identify the bay. A deactivated but
healthy drive: reseat the tray (hot-swap is supported), then Storage Manager >
Storage > pool > Repair. A dead drive: replace with a SATA HDD of at least the
smallest member's size, then Repair. Do not power off during the rebuild. The
alert resolves when `raidStatus` returns to 1 (it reads 2-10 during the
rebuild, which does not alert).
...
```promql
diskHealthStatus >= 3 or diskStatus == 4 or diskStatus == 5
```

**Means:** a specific bay reports SMART health Critical/Failing, or a system
partition failure or crash on that disk. Usually precedes a degrade.

**First three checks:**

```bash
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=diskHealthStatus'
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=diskStatus'
doppler run --project dhg-monitoring --config dev -- bash -c 'sshpass -e ssh -p 28 "$SYNOLOGY_OPS_USER@$SYNOLOGY_HOST" "printf \"%s\\n\" \"\$SSHPASS\" | sudo -S -p \"\" smartctl -H -A /dev/<bay device> | grep -E \"overall-health|Reallocated|Pending|Uncorrectable\""'
```

**Likely causes:** reallocated or pending sectors climbing; a drive past its
service life (the 6 TB IronWolfs in this NAS have 50k+ power-on hours); a bad
cable/backplane showing as CRC errors.

**Resolve:** order the replacement before the drive fails outright; replace and
Repair while the pool still has redundancy. Keep the drill cadence: a degraded
pool is exactly when the off-host copy matters.
...
**Likely causes:** ActiveBackup, Time Machine, Plex or a personal share growing;
snapshot retention on other folders; not the backup mirror unless retention
broke locally (check `bl_prune_local` ran: no run dirs older than 84 days).

**Resolve:** DSM Storage Manager > Storage Analyzer to find the consumer. If
the mirror itself grew, the local prune is the fix (the NAS is a pure mirror).
...
**Means:** a drive has been above 50 °C for 15 minutes. Sustained heat
shortens drive life and precedes SMART failures.

**First three checks:**

```bash
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=diskTemperature'
curl -sG http://10.0.0.251:9090/api/v1/query --data-urlencode 'query=raidStatus'    # a rebuild runs the drives hot
doppler run --project dhg-monitoring --config dev -- bash -c 'sshpass -e ssh -p 28 "$SYNOLOGY_OPS_USER@$SYNOLOGY_HOST" cat /sys/class/thermal/thermal_zone*/temp 2>/dev/null'
```

**Likely causes:** a RAID rebuild or data scrub (expected, transient); a fan
failure or dust; the rack/closet ambient temperature; a drive going bad.

**Resolve:** if a rebuild is running, wait it out. Otherwise check DSM >
Control Panel > Hardware & Power fan mode and the physical airflow. A single
drive hot while its neighbours are not is a drive problem, not a cooling one.
...
{/* automation:begin LokiStoreGrowth */}
**Automation** — `observability/runbooks/LokiStoreGrowth.yml`, trigger `T15`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Store size and its textfile freshness (a frozen value can masquerade as growth) | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=loki_store_bytes or node_textfile_mtime_seconds'` |
| 2 | Top talkers over the last hour | `curl -sG http://dhg-loki:3100/loki/api/v1/query --data-urlencode 'query=topk(10, sum by (container) (count_over_time({job="dhg-ai-factory"}[1h])))'` |
| 3 | /mnt/4tb headroom | `curl -sG http://dhg-prometheus:9090/api/v1/query --data-urlencode 'query=node_filesystem_avail_bytes{mountpoint="/mnt/4tb"}'` |
{/* automation:end */}
...
{/* automation:begin LokiDown */}
**Automation** — `observability/runbooks/LokiDown.yml`, trigger `T17`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Container state, exit code and restart count | `docker inspect dhg-loki --format '{{.State.Status}} exit={{.State.ExitCode}} restarts={{.RestartCount}} started={{.State.StartedAt}}'` |
| 2 | Last 200 log lines | `docker logs --tail 200 dhg-loki` |
| 3 | Readiness endpoint from inside the network | `curl -s -m 5 http://dhg-loki:3100/ready` |

Notes: The 3.x image has no shell, so this alert is the liveness probe.
{/* automation:end */}
...
{/* automation:begin AlloyDown */}
**Automation** — `observability/runbooks/AlloyDown.yml`, trigger `T16`, mode `notify`. When this alert opens an incident, dhg-remediator runs the read-only diagnostics below once per incident state change and records each result as an action on the incident (Actions tab). Nothing is restarted, stopped, or changed.

| # | Diagnostic | Command |
|---|---|---|
| 1 | Container state, exit code and restart count | `docker inspect dhg-alloy --format '{{.State.Status}} exit={{.State.ExitCode}} restarts={{.RestartCount}} started={{.State.StartedAt}}'` |
| 2 | Last 100 log lines (a bad config.alloy is named here) | `docker logs --tail 100 dhg-alloy` |
| 3 | Is Loki accepting writes (Alloy's downstream) | `curl -s -m 5 http://dhg-loki:3100/ready` |
| 4 | Alloy readiness endpoint | `curl -s -m 5 http://dhg-alloy:12345/-/ready` |
{/* automation:end */}
