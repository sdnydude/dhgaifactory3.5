# How to Generate All 4 Agent Build Prompts via Anthropic API

## Why API vs. Claude.ai

| | Claude.ai (what you're using now) | Anthropic API |
|---|---|---|
| **Context window** | 200K but conversation overhead eats into it | Full 200K dedicated to your task |
| **Output length** | ~4,000-6,000 tokens per message, then "continue" | Up to **128K tokens per response** (Opus 4.5) |
| **Rate limiting** | Message caps per day/hour | Token-based, scales with spend tier |
| **Automation** | Manual copy-paste | Script runs all 4 agents back-to-back |
| **Input control** | Can't precisely control system prompt + context | Full control over system prompt, temperature, etc. |
| **Cost** | $20/mo Pro or $100/mo Max | Pay per token (~$8-25 for all 4 agents) |

**Bottom line:** A 15,000-word agent prompt is ~20,000 tokens of output. In claude.ai you'd need 4-5 "continue" messages per agent. Via API, each agent generates in a single call.
...
2. Sign up or log in (separate from your claude.ai account)
3. Add a payment method (Settings → Billing)
4. Create an API key (Settings → API Keys → Create Key)
5. Add $25 of credits to start (enough for all 4 agents with room to iterate)
...
| Model | Input (per 1M tokens) | Output (per 1M tokens) | Est. Cost for All 4 Agents |
|---|---|---|---|
| **Claude Opus 4.5** | $5.00 | $25.00 | ~$15-25 (highest quality) |
| **Claude Sonnet 4.5** | $3.00 | $15.00 | ~$8-15 (recommended) |

Each agent: ~10K input tokens (system prompt + build plan) + ~25K output tokens (the build prompt).
Total across 4 agents: ~40K input + ~100K output ≈ **$8-15 with Sonnet 4.5**.

---

## The Script

Save this as `generate_agent_prompts.py` and run it:
...
### Generate all 4 agents (Sonnet 4.5 — recommended first pass):

```bash
python generate_agent_prompts.py
```

### Generate just Agent 1 to test:

```bash
python generate_agent_prompts.py --agent 1
```

### Generate all 4 with Opus 4.5 (highest quality, ~2x cost):

```bash
python generate_agent_prompts.py --model opus
```

### Recommended workflow:

```bash
# 1. Test with Agent 1 on Sonnet first (~$2-3)
python generate_agent_prompts.py --agent 1

# 2. Review the output — is it complete? Does it meet the quality bar?
cat output/agent-01-platform-qa-langgraph.md | head -100

# 3. If good, run all 4
python generate_agent_prompts.py

# 4. If you want higher quality, re-run specific agents on Opus
python generate_agent_prompts.py --agent 1 --model opus
```
...
```
output/
├── agent-01-platform-qa-langgraph.md      (~15,000 words)
├── agent-02-roi-calculator-langgraph.md   (~14,000 words)
├── agent-03-workflow-designer-langgraph.md (~13,000 words)
└── agent-04-use-case-match-langgraph.md   (~12,000 words)
```

Each file is a complete build prompt ready to paste into Gemini 2.5 Pro.

---

## After Generation: Feed to Gemini

Once you have the 4 markdown files, feed each one to Gemini 2.5 Pro (via AI Studio or API) to generate the actual agent code:

```bash
# Option A: Google AI Studio (browser)
# Go to aistudio.google.com → paste the markdown → generate
...
prompt = open('output/agent-01-platform-qa-langgraph.md').read()
...
| Issue | Fix |
|---|---|
| `anthropic.AuthenticationError` | Check your API key is set correctly |
| `anthropic.RateLimitError` | Wait 60 seconds and retry, or upgrade usage tier |
| Output truncated | Increase `MAX_OUTPUT_TOKENS` (current: 65536) |
| Output has placeholders | Re-run with `--model opus` for higher quality |
| `pip install anthropic` fails | Try `pip3 install anthropic` or use a virtualenv |
